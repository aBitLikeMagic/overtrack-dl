...
exports.makeServer = () => {
    ...
    app.post('/download-personal', (request, response) => __awaiter(this, void 0, void 0, function* () {
        yield overtrack_dl_1.OvertrackUser.getGamesWithData(request.body.session);
        response.redirect(`/?session=${request.body.session}`);
    }));
    app.post('/download-shared', (request, response) => __awaiter(this, void 0, void 0, function* () {
        try {
            yield overtrack_dl_1.OvertrackUser.getGamesWithData(undefined, request.body.shareKey);
    ...
    app.get('/graph/:name/5', (request, response) => __awaiter(this, void 0, void 0, function* () {
        ...
        if (specifiedName === 'magic') {
            ...
            if (specifiedNames.includes('magic'))
                games['magic'] = [2169, 2150, 2193, 2232, 2272, 2253, 2238, 2276, 2311, 2292, 2322, 2358, 2390, 2367, 2399, 2434, 2413, 2443, 2469, 2493, 2470, 2495, 2472, 2450];
            if (specifiedNames.includes('might'))
                games['might'] = [2303, 2286, 2264, 2250, 2236, 2217, 2198, 2234, 2215, 2257, 2296, 2273, 2309, 2290, 2322, 2304, 2328, 2357, 2376, 2404, 2382, 2365, 2343, 2343, 2369, 2394, 2369, 2397, 2376, 2404, 2427, 2399, 2378, 2402, 2434, 2460, 2437, 2415, 2392, 2368, 2345, 2366, 2341, 2365, 2365, 2390, 2371, 2348, 2325, 2305, 2336, 2314, 2335, 2357, 2336, 2363];
            if (specifiedNames.includes('mogul'))
                games['mogul'] = [2046, 1996, 2054, 2054, 2117, 2163, 2220, 2188, 2225, 2267, 2237, 2268, 2248, 2224, 2204, 2185, 2210, 2189, 2169, 2196, 2176, 2153, 2134, 2164, 2192, 2169, 2151, 2179, 2198, 2223, 2250, 2271, 2296, 2316, 2340, 2313, 2287, 2310, 2337, 2364, 2385, 2363, 2384, 2361, 2340, 2314, 2337, 2312, 2310, 2287, 2312, 2290, 2267, 2289, 2268, 2246, 2220, 2245, 2221, 2199, 2220, 2242, 2222, 2244, 2266, 2245, 2269, 2244, 2272, 2300, 2275, 2251, 2272, 2295, 2318, 2293, 2273, 2251, 2276, 2244, 2221, 2240, 2217, 2241, 2216, 2242, 2267, 2290, 2313, 2292, 2312, 2340, 2340, 2367, 2394, 2418, 2391, 2365, 2343, 2320, 2342, 2368, 2343, 2320, 2343, 2321, 2342, 2322, 2344, 2371];
            if (specifiedNames.includes('magoo'))
                games['magoo'] = [2546, 2623, 2681, 2636, 2697];
        ...
        for (const filename of yield fs.readdir('games')) {
            ...
            if (match && allowedNames.has(match[1])) {
                ...
                if (!labels[name])
                    labels[name] = [];
                const data = JSON.parse(yield fs.readFile('games/' + filename, 'utf8'));
